 ...
 * <code>ClusterExample</code>s
 * 
 * @author Danilo Croce
 * 
 */
public class ClusterExampleTypeResolver implements TypeIdResolver {
	private static Logger logger = LoggerFactory
			.getLogger(ClusterExampleTypeResolver.class);

	private static Map<String, Class<? extends ClusterExample>> idToClassMapping;
	private static Map<Class<? extends ClusterExample>, String> classToIdMapping;

	static {
		Reflections reflections = new Reflections("it");
		idToClassMapping = new HashMap<String, Class<? extends ClusterExample>>();
		classToIdMapping = new HashMap<Class<? extends ClusterExample>, String>();
		Set<Class<? extends ClusterExample>> classes = reflections
				.getSubTypesOf(ClusterExample.class);
		for (Class<? extends ClusterExample> clazz : classes) {
		...
		logger.debug("ClusterExample Implementations: {}", idToClassMapping);
	}
	...
	public JavaType typeFromId(String arg0) {
		...
		Class<? extends ClusterExample> clazz = idToClassMapping.get(arg0);
		if (clazz != null) {
