 ...
 * the serialization in JSON and deserialization of <code>Label</code>s 
 * 
 * @author Simone Filice
 *
 */
public class LabelTypeResolver implements TypeIdResolver{
	private static Logger logger = LoggerFactory.getLogger(LabelTypeResolver.class);

	private static Map<String, Class<? extends Label>> idToClassMapping;
	private static Map<Class<? extends Label>, String> classToIdMapping;

	static{
		Reflections reflections = new Reflections("it");
		idToClassMapping = new HashMap<String, Class<? extends Label>>();
		classToIdMapping = new HashMap<Class<? extends Label>, String>();
		Set<Class<? extends Label>> classes = reflections.getSubTypesOf(Label.class);
		...
		logger.debug("Label Implementations: {}", idToClassMapping);
	}
	...
	public JavaType typeFromId(String arg0) {
		...
		Class<? extends Label> clazz = idToClassMapping.get(arg0);
		if(clazz!=null){
