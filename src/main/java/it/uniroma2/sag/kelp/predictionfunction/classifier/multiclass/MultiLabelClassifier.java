...
/**
 * It is a multi label classifier. It is suitable for multi label 
 * classification tasks where every example can be associated to zero, one or multiple classs.
 * Basically, given a multi label classification task with C classes, starting from the output 
 * provided by C different binary classifiers (on which this classifier relies on) operating in
 * a 1-vs-All Schema , the final predicted classes are the ones associated to positive scores
 * 
...
public class MultiLabelClassifier implements Classifier{
	...
	public MultiLabelClassificationOutput predict(Example example) {
		MultiLabelClassificationOutput finalOutput = new MultiLabelClassificationOutput();
		for(Classifier classifier : binaryClassifiers){
